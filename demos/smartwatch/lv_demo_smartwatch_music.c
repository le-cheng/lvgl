...
void lv_demo_smartwatch_music_create(void)
{
    ...
    LV_LOG_WARN("Aligned pos x: %ld, y: %ld", lv_pct(10), lv_pct(25));

    LV_LOG_WARN("Volume pos x: %ld, y: %ld", lv_obj_get_x_aligned(image_volume), lv_obj_get_y_aligned(image_volume));
...
static void rotation_complete_cb(lv_anim_t * a)
{
    ...
    if(end_angle % 90 != 0) {
        ...
        LV_LOG_WARN("end angle: %ld", end_angle);
